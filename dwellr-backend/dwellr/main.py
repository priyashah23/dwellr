...
def migration() -> None:
    ...
    return '<p> Migration Attempted </p>'
...
def migrate_properties():
    ...
    cursor.close()
    conn.close()
    return ('<p> Migration WBASIUDBAISUD </p>')
...
def create_user():
    ...
    cursor.execute('''
        INSERT IGNORE INTO user (name, age) VALUES (%s, %s)
    ''', name, age)
...
def post_preferences():
    ...
    number_of_beds = preferences["is_new_home"]
    location = preferences["location"]

    try: 
        cursor.execute(''''
            INSERT INTO preferences (user_id, max_price, number_of_beds, is_new_home, location)
            VALUES (%s, %s, %s, %s, %s)
        ''',user_id, max_price, number_of_beds, location)
    ...
    preferences = {"number_of_bedrooms": 2, "number_of_bathrooms": 3}
